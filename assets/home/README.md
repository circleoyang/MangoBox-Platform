...
對應 index.html 的 data-image 欄位。以下是建議原始素材尺寸（寬 × 高，px），不是 CSS 固定顯示尺寸。首屏 mangobox-hero.png 已採用使用者確認的六人教室插圖（實際 1448 × 1086，4:3），保留原始 PNG；首屏改為滿版背景，顯示時使用 object-fit:cover 依螢幕比例裁切，搭配霧白漸層確保文字可讀。其餘 10 張圖片維持預留區，待使用者提供後整合，不載入不存在的圖片。
...
- 產品與套件圖片以 object-fit:contain 整合，不因卡片長寬裁掉 PCB 或套件內容。首屏情境插圖例外，作為裝飾性滿版背景使用 object-fit:cover，手機加深遮罩。
